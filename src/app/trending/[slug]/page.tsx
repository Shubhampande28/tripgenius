...
export async function generateStaticParams() {
  return getAllTrending().map(a => ({ slug: a.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const article = getTrendingBySlug(slug);
  if (!article) return {};
  return {
    title: article.title,
    description: article.description,
    alternates: { canonical: `${BASE}/trending/${slug}` },
    openGraph: {
      title: article.title,
      description: article.description,
      url: `${BASE}/trending/${slug}`,
      type: 'article',
...
// Renders the markdown inline subset the generator produces:
...
function ContentBlock({ block }: { block: TrendingBlock }) {
  switch (block.type) {
    case 'h2':
...
export default async function TrendingArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const article = getTrendingBySlug(slug);
  if (!article) notFound();

  const more = getAllTrending().filter(a => a.slug !== slug).slice(0, 3);
  const articleUrl = `${BASE}/trending/${slug}`;
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Trending', item: `${BASE}/trending` },
    ...
    <>
      ...
      <main className="bg-dark min-h-screen">
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 -mt-16 relative z-10 pb-20">
          ...
          {/* Sources — when the generator grounded facts in real reporting */}
          ...
          {more.length > 0 && (
            <div className="mt-12">
              <h2 className="font-heading text-xl font-semibold text-primary-text mb-5">More trending reads</h2>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {more.map(a => (
                  <Link key={a.slug} href={`/trending/${a.slug}`}
