...
# FAQ Completeness — GEO/AI-Search Pass (2026-07-07)

City pages now guarantee ≥4 FAQs by supplementing hand-authored FAQs with
answers derived from each city's authored structured data (best time, budget,
currency, language, visa) via `derivedFaqs()` in `src/lib/cityFaqs.ts`.

## Cities with insufficient data for FAQs (intentionally skipped)

- **All ~233 non-indexable city pages** (stubs without hand-authored
  month-by-month data, per `isIndexableCity`) render **no FAQs**. Their stats
  are template-generated by `enrichCity`, so deriving FAQs from them would
  publish fabricated facts — the exact "low-value content" pattern the AdSense
  remediation removed. They are also `noindex`, so AI engines never see them.
  If any of these cities gets authored data later, FAQs appear automatically.

## Derived FAQ answers worth a human skim

- The 64 indexable cities without hand-authored FAQ sets now show 4–5
  data-derived FAQs each. Answers restate `city.stats` and `monthByMonth`
  values verbatim — verify those source fields are current (esp. `budget`
  ranges and `visa` strings written in 2025).
